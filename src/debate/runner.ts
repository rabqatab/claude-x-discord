 ...
 *  When Claude uses tools (file reads etc.), `result` is often a brief summary
 *  while the full analysis lives in content_block_delta events.
 */
function extractClaudeResult(output: string): string {
  ...
  // Prefer the most complete content:
  // delta text captures all streaming output; result is the CLI's final summary.
